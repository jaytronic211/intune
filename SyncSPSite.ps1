#Description:		Sync SharePoint site to desktop using onedrive. Runs on login.
#Modified from https://www.lieben.nu/liebensraum/2019/06/simplify-logon-scripts-in-intune-without-schedules
...
if($runningAsSystem){
    ...
    if($visibleToUser){
        $res = [murrayju.ProcessExtensions]::StartProcessAsCurrentUser($targetUserSessionId, "c:\windows\system32\WindowsPowerShell\v1.0\powershell.exe", " -WindowStyle Normal -nologo -executionpolicy ByPass -Command `"& '$scriptPath' '$regPath'`"",$True)
    }else{
        $res = [murrayju.ProcessExtensions]::StartProcessAsCurrentUser($targetUserSessionId, "c:\windows\system32\WindowsPowerShell\v1.0\powershell.exe", " -WindowStyle Hidden -nologo -executionpolicy ByPass -Command `"& '$scriptPath' '$regPath'`"",$False)
    }
    ...
    $acl = Get-Acl $regPath
    $acl.SetAccessRule( (New-Object System.Security.AccessControl.RegistryAccessRule "BUILTIN\Users", "FullControl", "ContainerInherit", "None", "Allow") )
    $acl | Set-Acl
...
ac (Join-Path "$env:TEMP" "IntuneLogonScript.log") "[$(Get-Date)] ***** Starting User Logon script ($env:USERNAME logged on)."
$regPath = $args[0]

$runOnceEntries = (Get-ItemProperty -path "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce")
if( ([Array]@($runOnceEntries.PSObject.Properties.Name | % {if($runOnceEntries.$_ -eq "reg delete $($regPath.Replace(":",'')) /f"){$_}})).Count -le 0){
    New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\RunOnce" -Name $(Get-Random) -Value "reg delete $($regPath.Replace(":",'')) /f" -PropertyType String -Force -ErrorAction SilentlyContinue
}

#################################YOUR CODE BELOW#####################################

ac (Join-Path "c:\temp" "test.txt") "Sync SP for $($Env:USERNAME) ran at $(Get-Date)"
...
try {
	...
	$OrganisationDisplayName = "xxxxxxx"
    ...
    if (!(Test-Path $($params.syncPath))) {
        ...
        ac (Join-Path "c:\temp" "test.txt") "Sharepoint folder not found locally, will now sync.. $(Get-Date)"
        $sp = Sync-SharepointLocation @params
        if (!($sp)) {
        ac (Join-Path "c:\temp" "test.txt") "Sharepoint sync failed. $(Get-Date)"
            Throw "Sharepoint sync failed."
        }
    } else {
        Write-Host "Location already syncronized: $($params.syncPath)" -ForegroundColor Yellow
	ac (Join-Path "c:\temp" "test.txt") "Location already syncronized: $($params.syncPath) $(Get-Date)"
    }
    #endregion
} catch {
    $errorMsg = $_.Exception.Message
...
ac (Join-Path "c:\temp" "test.txt") "Completed successfully.. $(Get-Date)"
}
}
#endregion
